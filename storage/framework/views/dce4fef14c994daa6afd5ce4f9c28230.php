        ...
        $.ajax({
            ...
            success: function(data) {
                if (data) {
                    ...
                    if (need_otp == 'Enabled' && need_otp_user == 'Enabled') {
...
function loginWithoutOTP() {
    ...
    $.ajax({
        ...
        success: function(data) {
            if (data.success) {
                stepper1.next();
                waitToGoDashboard(data.redirect_url);
            } else {
                $('#password').addClass('is-invalid');
                document.getElementById('password_error').innerHTML = data.message || 'Please enter a correct password.'
            }
        },
        error: function(err) {
            $('#password').addClass('is-invalid');
            document.getElementById('password_error').innerHTML = 'Login failed. Please try again.'
        }
    });
}
...
function waitToGoDashboard(redirectUrl) {
    setTimeout(function() {
        sendToDashboardNow(redirectUrl);
    }, 5000)
}

function sendToDashboardNow(redirectUrl) {
    if (redirectUrl) {
